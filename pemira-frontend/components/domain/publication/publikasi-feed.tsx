...
const STAT_CARDS = [
  { key: "masuk", label: "Laporan Masuk", icon: Inbox, wrap: "bg-amber/15 text-ochre", value: "text-steel-ink" },
  { key: "diinvestigasi", label: "Sedang Diinvestigasi", icon: Activity, wrap: "bg-warning/15 text-ochre", value: "text-ochre" },
...
export function PublikasiFeed() {
  ...
  return (
    <>
      {/* Dashboard statistik */}
      <section className="border-b border-canvas-line bg-canvas py-14">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-8 flex items-center justify-center gap-2">
            <span className="size-2 animate-pulse rounded-full bg-success" aria-hidden />
            <span className="text-xs font-semibold tracking-[0.2em] text-ink-muted uppercase">
              Rekapitulasi Kasus
            </span>
          </div>
          ...
          <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {STAT_CARDS.map((card) => (
              <li
                key={card.key}
                className="rounded-2xl border border-steel/20 bg-surface p-6 shadow-sm"
              >
                <span
                  className={cn(
      ...
      <section className="bg-ivory py-16 lg:py-20">
        <div className="mx-auto max-w-5xl px-6">
          ...
          {loading ? (
            ...
            <p className="mt-12 rounded-2xl border border-steel/20 bg-surface p-8 text-center text-ink-muted shadow-sm">
            ...
            <ul className="mt-12 space-y-6">
              {items.map((pub) => (
                <li
                  key={pub.slug}
                  className="rounded-2xl border border-steel/20 bg-surface p-6 shadow-sm transition-colors hover:border-steel/50 sm:p-8"
                >
                  ...
                  <Link href={`/publikasi/${pub.slug}`}>
                    <h3 className="mt-4 text-xl font-bold text-steel-ink hover:text-steel-deep">
                      {pub.title}
                    </h3>
                  </Link>
                  <p className="mt-3 leading-relaxed text-ink-muted">{pub.summary}</p>

                  <div className="mt-5 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-canvas-line pt-5 text-sm">
                    {pub.reportedCandidate && (
                      <span className="text-ink-muted">
                        Terlapor:{" "}
                        <span className="font-medium text-steel-ink">
                          {pub.reportedCandidate}
                        </span>
                      </span>
                    )}
                    ...
                    {pub.instagramUrl && (
                      <a
                        ...
                        className="ml-auto inline-flex items-center gap-1.5 font-semibold text-steel-deep transition-colors hover:text-steel-ink"
                      >
                        Lihat di Instagram
